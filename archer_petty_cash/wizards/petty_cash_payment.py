...
class PettyCashPaymentRegister(models.TransientModel):
    _name = 'archer.petty.cash.payment.register'

    request_id = fields.Many2one(comodel_name='archer.petty.cash.request')
    ...
    def action_confirm_payment(self):
        ...
        payment = self.env['account.payment'].create({
            ...
            'ref': 'Petty Cash Request %s'%self.request_id.sequence,
            'for_petty_cash':True,
            'request_id':self.request_id.id,
            'analytic_account_id':self.request_id.project_id.analytic_account_id.id,
        })
        self.request_id.write({'payment_id':payment.id,'state':'in_payment'})
