...
class BusinessTrip(http.Controller):
    @validate_token
    @http.route("/api/requests/create_business_trip_request", methods=["POST"], type="json", auth="none", csrf=False)
    def create_business_trip_request(self, **kw):
        try:
            employee_id = request.env['hr.employee'].sudo().browse(kw['employee_id'])
            kw['project_id'] = employee_id.project_id.id
            kw['state'] = 'submit'
            business_trip_id = request.env['business.trip'].sudo().create(kw)
            handle_create_fcm_msg(business_trip_id)
