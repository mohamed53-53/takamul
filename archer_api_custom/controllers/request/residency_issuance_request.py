...
class ResedincyIssuance(http.Controller):
    @validate_token
    @http.route("/api/requests/create_residency_issuance_request", methods=["POST"], type="json", auth="none", csrf=False)
    def create_residency_issuance_request(self, **kw):
        try:
            ...
            kw['request_type'] = 'residency_issuance'
            kw['state'] = 'submit'
            if residency_id.residency_type == 'employee':
            ...
            residency_issuance_id = request.env['residency.issuance'].sudo().create(kw)
            handle_create_fcm_msg(residency_issuance_id)
            response = Response(
                status=200,
    ...
    @validate_token
    @http.route("/api/requests/get_employee_residency_issuance_request", methods=["POST"], type="json", auth="none", csrf=False)
    def get_employee_residency_issuance_request(self, **kw):
        try:
            domain = [('request_type', '=', 'residency_issuance'), ('employee_id', '=', kw['employee_id'])]
            if 'request_id' in kw:
