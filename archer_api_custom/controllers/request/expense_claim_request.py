...
class ExpenseClaim(http.Controller):
    @validate_token
    @http.route("/api/requests/create_expense_claim_request", methods=["POST"], type="json", auth="none", csrf=False)
    def create_expense_claim_request(self, **kw):
        try:
            employee_id = request.env['hr.employee'].sudo().browse(kw['employee_id'])
            kw['project_id'] = employee_id.project_id.id
            kw['state'] = 'submit'
            expense_claim_id = request.env['expense.request'].sudo().create(kw)
            handle_create_fcm_msg(expense_claim_id)
            response = Response(
                status=200,
