...
class BenefitRequest(http.Controller):
    @validate_token
    @http.route("/api/requests/create_benefit_request_request", methods=["POST"], type="json", auth="none", csrf=False)
    def create_benefit_request_request(self, **kw):
        try:
            employee_id = request.env['hr.employee'].sudo().browse(kw['employee_id'])
            kw['project_id'] = employee_id.project_id.id
            kw['state'] = 'submit'
            benefit_id = request.env['benefit.request'].sudo().create(kw)
            handle_create_fcm_msg(benefit_id)
            response = Response(
                status=200,
