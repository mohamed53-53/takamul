...
class ResidencyJobTitleRequest(http.Controller):
    @validate_token
    @http.route("/api/requests/create_residency_job_title_request", methods=["POST"], type="json", auth="none", csrf=False)
    def create_residency_job_title_request(self, **kw):
        try:
            employee_id = request.env['hr.employee'].sudo().browse(kw['employee_id'])
            kw['project_id'] = employee_id.project_id.id
            kw['state'] = 'submit'
            residency_job_title_id = request.env['residency.job.title'].sudo().create(kw)
            handle_create_fcm_msg(residency_job_title_id)
            response = Response(
                status=200,
