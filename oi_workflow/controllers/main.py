...
class UserAudit(http.Controller):
    ...
    def record_info(self, model, record_id):
        ...
        data = {
            'lines' : [],
            'show_login_as' : request.env.user._is_system() and request.env['ir.module.module']._installed().get('oi_login_as')  # @UndefinedVariable
            }
    ...
    @http.route('/oi_workflow/approval_settings', type='json', auth='user')
    def approval_settings(self):
        return {
            ...
            'state_models' : request.env.user._is_system() and request.env['ir.model.fields'].search([('name','=', 'state'),('ttype','=', 'selection'), ('store','=', True)]).mapped('model') or []  # @UndefinedVariable
            }    
