...
class tds_accounts_wizard(models.TransientModel):
    ...
    currency_id = fields.Many2one('res.currency', string='Currency', required=True,
                                  default=lambda self: self.env.user.company_id.currency_id)
    reg_id = fields.Many2one('account.payment.register', string='Payment Record')
...
class AccountPayment(models.Model):
    ...
    tds = fields.Boolean(string='TDS / Withholding', default=False)
    tds_tax_ids = fields.Many2many('account.tax', string='TDS')
    tds_amt = fields.Float(string='TDS Amount', compute='compute_tds_amnt', store=True)
        ...
        if self.tds:
            for rec in self:
                ...
                if rec.payment_type == 'inbound':
                    return {'domain': {
                        'tds_tax_ids': [('type_tax_use', '=', 'sale'), ('tds', '=', True)]}}
                else:
                    return {'domain': {
                        'tds_tax_ids': [('type_tax_use', '=', 'purchase'), ('tds', '=', True)]}}
    ...
    def onchange_tds_tax_ids(self):
        ...
        applicable = False
        for tds_tax in self.tds_tax_ids:
    ...
    @api.depends('tds', 'tds_tax_ids', 'amount')
    def compute_tds_amnt(self):
        for payment in self:
            payment.tds_amt = 0.0
            if payment.tds and payment.tds_tax_ids and payment.amount:
                applicable = True
                total_tds_tax_amount = 0.0
                for tax in payment.tds_tax_ids:
                    if payment.partner_id and payment.partner_id.tds_threshold_check:
                        for lines in self:
                            applicable = lines.check_turnover(lines.partner_id.id, tax.payment_excess, lines.amount)
                    if applicable:
                        taxes = tax._origin.compute_all(
                            payment.amount)
                        total_tds_tax_amount += taxes['total_included'] - taxes[
                            'total_excluded'] if taxes else 0.0
                        payment.tds_amt = total_tds_tax_amount
                    else:
                        payment.tds_amt = 0.0
            else:
                payment.tds_amt = 0.0
            ...
            if self.currency_id != self.company_id.currency_id:
                if self.payment_type == 'inbound' and self.partner_type == 'customer':
                    ...
                    if self.env.context.get('tds_amount'):
                        ...
                        converted_tds_amt = self.currency_id._convert(self.tds_amt,
                                                                      self.company_id.currency_id,
                                                                      self.company_id, self.date)
                        tds_amount = self.tds_amt

                    print('converted_amt', converted_amt, converted_tds_amt, tds_amount)
                    res[1]['credit'] = converted_amt - converted_tds_amt
                    res[1]['amount_currency'] = self.tds_amt - self.amount
                    credit = converted_tds_amt
                    if self.env.context.get('tds_tax_ids'):
                        for lines in self.env.context.get('tds_tax_ids'):
...
class AccountPaymentRegister(models.TransientModel):
    ...
    tds = fields.Boolean(string='TDS / Withholding', default=False)
    tds_tax_ids = fields.Many2many('account.tax', string='TDS')
    tds_amt = fields.Float(string='TDS Amount', compute='compute_tds_amount')
    tax_amount = fields.Float(string='TAX Amount')
    vendor_type = fields.Selection(related='partner_id.company_type', string='Partner Type')
    tds_multi_acc_ids = fields.One2many('tds.accounts.wizard', 'reg_id', string='Write Off Accounts')

    def _create_payments(self):
        if self.tds:
            context = self.env.context.copy()
            context.update({'TDS': True, 'tds_amount': sum(self.tds_multi_acc_ids.mapped('amount')),
                            'tds_tax_ids': self.tds_tax_ids})
            self.env.context = context
            res = super(AccountPaymentRegister, self)._create_payments()
            for ids in self.tds_tax_ids:
                res.tds_tax_ids = ids
            res.tds = True
            res.tds_amt = self.payment_difference
            if self.currency_id != self.company_id.currency_id:
                tds_amount = sum(self.tds_multi_acc_ids.mapped('amount'))
                tds_amount = self.company_id.currency_id._convert(tds_amount, self.currency_id, self.company_id,
                                                                  self.payment_date)
                res.tds_amt = tds_amount
            return res
        else:
    ...
    def _create_payment_vals_from_wizard(self):
        ...
        if not self.currency_id.is_zero(self.payment_difference) and self.tds:
            write_off_vals = []
            for woff_payment in self.tds_multi_acc_ids:
                write_off_vals.append({
                    'name': woff_payment.name,
                    'amount': woff_payment.amount or 0.0,
                    'account_id': woff_payment.tds_account_id.id,
                    'is_multi_write_off': True
                })
                if not res['write_off_line_vals']['account_id']:
                    res['write_off_line_vals']['account_id'] = woff_payment.tds_account_id.id
    ...
    def onchange_tds_tax_ids(self):
        ...
        for line in self.tds_multi_acc_ids:
            diff_amount = sum([line.amount for line in self.tds_multi_acc_ids])
        self.write({
            'amount': amount_res - diff_amount,
            'tds_amt': diff_amount,
    ...
    def compute_tds_amount(self):
        for payment in self:
            ...
            if payment.tds and payment.tds_tax_ids and payment.amount:
                ...
                for tax in payment.tds_tax_ids:
                    if payment.partner_id and payment.partner_id.tds_threshold_check:
                        applicable = self.check_turnover(self.partner_id.id, tax.payment_excess, self.amount)
                    if applicable:
                        taxes = tax._origin.compute_all(
                            payment.amount)
                        total_tds_tax_amount += taxes['total_included'] - taxes[
